...
**买入信号** (默认 score ≥ 65 触发):
```
score = volumeBurst × 0.40        # 量能爆发(5x基线)
      + walletStructure × 0.35    # 买家暴增 + 新钱包占比 + 健康买单大小
      + priceStructure × 0.15     # 价格走势 + 大单分布
      + safety × 0.10             # 持币地址趋势
```
...
3. `take_profit_2` - 涨至 +200% (3x), **100%清仓** (TP2先于TP1判断,处理暴涨场景)
4. `take_profit_1` - 涨至 +100% (2x), 卖50%
5. `large_sell` - 1分钟内大单卖出冲击 ≥ 5% LP
6. `volume_reversal` - 已盈利 ≥25% 且连续2分钟净流出
